...
export default function AppliedJobsPage() {
    ...
    const appliedJobs: AppliedJob[] = [
        ...
        },
        {
            ...
            companyRating: 4.4,
            companyReviews: 187,
            nextSteps: 'Application not selected for this role',
    ...
    const statusFilters = [
        { value: 'all', label: 'All Applications', count: appliedJobs.length },
        {
            value: 'applied',
            label: 'Applied',
            count: appliedJobs.filter((job) => job.status === 'applied').length,
        },
        {
            value: 'reviewing',
            label: 'Under Review',
            count: appliedJobs.filter((job) => job.status === 'reviewing').length,
        },
        {
            value: 'interviewing',
            label: 'Interviewing',
            count: appliedJobs.filter((job) => job.status === 'interviewing').length,
        },
        {
            value: 'offer',
            label: 'Offer Received',
            count: appliedJobs.filter((job) => job.status === 'offer').length,
        },
        {
            value: 'rejected',
            label: 'Rejected',
            count: appliedJobs.filter((job) => job.status === 'rejected').length,
        },
        {
            value: 'withdrawn',
            label: 'Withdrawn',
            count: appliedJobs.filter((job) => job.status === 'withdrawn').length,
        },
    ];
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4 lg:p-6">
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900">
                                {appliedJobs.filter((job) => job.status === 'interviewing').length}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-xl bg-white p-4 shadow-sm">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900">
                                {appliedJobs.filter((job) => job.status === 'offer').length}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-xl bg-white p-4 shadow-sm">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900">
                                {appliedJobs.filter((job) => job.status === 'reviewing').length}
                            </p>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div className="mb-3 flex items-start justify-between">
                                        <div className="flex-1">
                                            <div className="mb-2 flex items-center space-x-3">
                                                ...
                                                <span
                                                    className={`inline-flex items-center rounded-full border px-3 py-1 text-sm font-medium ${getStatusColor(job.status)}`}
                                                >
                                    ...
                                    <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
                                        <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                                            ...
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span className="text-blue-800">
                                                    Updated {getTimeAgo(job.lastUpdated)}
                                                </span>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                        ...
                                        <div className="mb-4 rounded-lg border border-purple-200 bg-purple-50 p-3">
                                            ...
                                            <div className="grid grid-cols-2 gap-4 text-sm text-purple-700">
                                                <div>
                                                    ...
                                                    {job.interviewScheduled.date}
                                                </div>
                                                <div>
                                                    <span className="font-medium">Time:</span>{' '}
                                                    {job.interviewScheduled.time}
                                                </div>
                                                <div>
                                                    <span className="font-medium">Type:</span>{' '}
                                                    {job.interviewScheduled.type}
                                                </div>
                                                <div>
                                                    <span className="font-medium">
                                                        Interviewer:
                                                    </span>{' '}
                                                    {job.interviewScheduled.interviewer}
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                        ...
                                        <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-3">
                                            ...
                                            <div className="flex items-center space-x-4 text-sm text-gray-700">
                                                <div className="flex items-center space-x-2">
                                                    ...
                                                    <span>{job.recruiterContact.email}</span>
                                                </div>
                                                {job.recruiterContact.phone && (
                                                    <div className="flex items-center space-x-2">
                                                        <PhoneIcon className="h-4 w-4" />
                                                        <span>{job.recruiterContact.phone}</span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
            ...
            {sortedJobs.length === 0 && (
                <div className="py-12 text-center">
                    <CheckCircleIcon className="mx-auto mb-4 h-16 w-16 text-gray-400" />
